...
describe('SyncEngine', () => {
  ...
  describe('cascade delete', () => {
    // Helper: build a minimal CommonTask baseline entry
    function makeBaselineTask(uid: string, title: string, parentUid?: string): CommonTask {
      return {
        uid,
        title,
        status: 'TODO',
    ...
    it('parent deleted on Obsidian side cascades child deletes to CalDAV only', async () => {
      // Both parent and child exist in baseline and on CalDAV;
      // Obsidian has neither (user deleted both).
      // Diff emits direct toCalDAV deletes for both; cascade confirms both are covered.
      // Direction correctness: toObsidian must have no delete for c1.
      const idMapping = {
        taskIdToCaldavUid: { 'p1': 'caldav-uid-p1', 'c1': 'caldav-uid-c1' },
        caldavUidToTaskId: { 'caldav-uid-p1': 'p1', 'caldav-uid-c1': 'c1' },
      };
      mockGetIdMapping.mockReturnValue(idMapping);
      mockGetBaseline.mockReturnValue([
        makeBaselineTask('p1', 'Parent task'),
        makeBaselineTask('c1', 'Child task', 'p1'),
      ]);
      // CalDAV has both (RELATED-TO encodes parent relationship via VTODO mapper,
      // but the adapter resolves uids via idMapping; supply them as independent VTODOs
      // — the parentUid in baseline/tasksByUid is what expandSubtreeDeletes uses)
      mockFetchVTODOs.mockResolvedValue([
        makeCalObj('caldav-uid-p1', 'Parent task'),
        makeCalObj('caldav-uid-c1', 'Child task'),
      ]);
      // Obsidian: neither task is present
      mockGetAllTasksWithBody.mockResolvedValue([]);
      ...
      expect(result.success).toBe(true);
      // Both parent and child must be deleted on CalDAV side
      expect(result.deleted.toCalDAV).toBe(2);
      const caldavDeletedUids = result.details.toCalDAV
        .filter(c => c.type === 'delete')
        .map(c => c.task.uid);
      expect(caldavDeletedUids).toContain('p1');
      expect(caldavDeletedUids).toContain('c1');
      // Obsidian must not receive a delete for c1 (direction correctness)
      const obsidianDeletedUids = result.details.toObsidian
        .filter(c => c.type === 'delete')
        .map(c => c.task.uid);
      expect(obsidianDeletedUids).not.toContain('c1');
    });

    it('idMapping is cleaned up for every cascade-deleted descendant', async () => {
      // 3-level tree: p1 → c1 → g1 (grandchild)
      // All three deleted on Obsidian side; CalDAV still has them.
      // After sync, idMapping entries for all three must be gone.
      const idMapping = {
        taskIdToCaldavUid: {
          'p1': 'caldav-uid-p1',
          'c1': 'caldav-uid-c1',
      ...
      mockGetBaseline.mockReturnValue([
        makeBaselineTask('p1', 'Parent task'),
        makeBaselineTask('c1', 'Child task', 'p1'),
        makeBaselineTask('g1', 'Grandchild task', 'c1'),
      ]);
      mockFetchVTODOs.mockResolvedValue([
        makeCalObj('caldav-uid-p1', 'Parent task'),
        makeCalObj('caldav-uid-c1', 'Child task'),
        makeCalObj('caldav-uid-g1', 'Grandchild task'),
      ]);
      mockGetAllTasksWithBody.mockResolvedValue([]);
      ...
      expect(result.success).toBe(true);
      expect(result.deleted.toCalDAV).toBe(3);

      // idMapping must be cleaned up for all three cascade-deleted tasks
      expect(mockSetIdMapping).toHaveBeenCalled();
      const savedMapping = (mockSetIdMapping.mock.calls[0] as [IdMapping])[0];
      expect(savedMapping.taskIdToCaldavUid['p1']).toBeUndefined();
      expect(savedMapping.taskIdToCaldavUid['c1']).toBeUndefined();
      expect(savedMapping.taskIdToCaldavUid['g1']).toBeUndefined();
      expect(savedMapping.caldavUidToTaskId['caldav-uid-p1']).toBeUndefined();
      expect(savedMapping.caldavUidToTaskId['caldav-uid-c1']).toBeUndefined();
      expect(savedMapping.caldavUidToTaskId['caldav-uid-g1']).toBeUndefined();
    });

    // SKIPPED: "update + cascaded delete on the same task: delete wins"
    //
    // The scenario requires that a child task still present in Obsidian carries
    // parentUid='p1' in tasksByUid so expandSubtreeDeletes can cascade the parent
    // delete onto it.  In production, when a parent is deleted from Obsidian the
    // child no longer has a parent task reference; the ObsidianAdapter normalizes it
    // with parentUid=null.  tasksByUid is built with Obsidian tasks last (they
    // overwrite baseline), so c1's parentUid=null wins and the cascade cannot find
    // it as a descendant of p1.  The production code therefore never places both an
    // update and a cascaded delete for the same child in toCalDAV in this scenario
    // — the harness cannot express the precondition without changing production code.
    it.todo('update + cascaded delete on the same task: delete wins');
  });
