...
describe('ObsidianMapper', () => {
  ...
  describe('getContentHash', () => {
    it('should return trimmed original markdown', () => {
      ...
      expect(mapper.getContentHash(task)).toBe('- [ ] Task');
    });
  });
  ...
  describe('toTaskFields', () => {
    it('should reverse-map a TODO CommonTask', () => {
      const common: CommonTask = {
        uid: 'task-123', title: 'Buy groceries', status: 'TODO',
        dueDate: '2025-01-15', startDate: '2025-01-08',
        scheduledDate: '2025-01-10', completedDate: null,
        priority: 'high', tags: ['sync', 'shopping'], recurrenceRule: '', body: '',
      };

      const fields = mapper.toTaskFields(common);
    ...
    it('should reverse-map a DONE CommonTask', () => {
      const common: CommonTask = {
        uid: 'task-456', title: 'Done task', status: 'DONE',
        dueDate: null, startDate: null, scheduledDate: null,
    ...
    it('should reverse-map all priorities', () => {
      const base: CommonTask = {
        uid: 'id', title: 'T', status: 'TODO',
        dueDate: null, startDate: null, scheduledDate: null, completedDate: null,
        tags: [], recurrenceRule: '', body: '', priority: 'none',
      };

      expect(mapper.toTaskFields({ ...base, priority: 'highest' }).priority).toBe('1');
      expect(mapper.toTaskFields({ ...base, priority: 'high' }).priority).toBe('2');
      expect(mapper.toTaskFields({ ...base, priority: 'medium' }).priority).toBe('3');
      expect(mapper.toTaskFields({ ...base, priority: 'low' }).priority).toBe('5');
      expect(mapper.toTaskFields({ ...base, priority: 'lowest' }).priority).toBe('6');
      expect(mapper.toTaskFields({ ...base, priority: 'none' }).priority).toBe('0');
    });
  });
});
